...
class PlayAgeRangeDeclaration : HybridPlayAgeRangeDeclarationSpec() {
  ...
  private fun selectProvider(context: Context): AgeRangeDeclarationProvider = when {
    ...
    AgeRangeProviderStubOne.isAvailable(context)      -> AgeRangeProviderStubOne(context)
    AgeRangeProviderStubTwo.isAvailable(context)      -> AgeRangeProviderStubTwo(context)
